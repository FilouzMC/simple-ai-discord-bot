...
export const DEFAULT_CONFIG = {
  ...
  AskModel: {
    // Exemple: "🚀 Rapide": ["gemini-2.5-flash"], "🎯 Précis": ["gemini-2.5-pro"]
  },
  autoModel: {
    enabled: false,
    fallbackModels: []  // Liste de priorité des modèles à tenter, ex: ["gemini-2.5-flash", "gemini-2.5-pro", "sonar"]
  },
  channelContextMessageMaxAgeSeconds: 3600
  , autoSummaryEnabled: false
  , autoSummaryIdleSeconds: 1800
  , autoSummaryMinMessages: 15
  , autoSummaryPrompt: 'Tu es un assistant qui résume de manière structurée un débat sur Discord. Fournis: 1) Contexte global, 2) Points clés (puces), 3) Convergences, 4) Divergences, 5) Actions / décisions, 6) Questions ouvertes. Reste concis.'
  , autoSummaryContextLimit: 80
  , autoSummaryModel: ''
  // ── Autoprompt ──────────────────────────────────────────────────────────────
  // Activation globale du moteur autoprompt (les entrées individuelles ont leur propre "enabled")
...
// Réordonner selon AskModel si fourni
if (CONFIG.AskModel && typeof CONFIG.AskModel === 'object') {
  const ordered = [];
  const seen = new Set();
  for (const groupName of Object.keys(CONFIG.AskModel)) {
    const list = CONFIG.AskModel[groupName];
...
export function getAutoModelConfig() {
  return CONFIG.autoModel || { enabled: false, fallbackModels: [] };
}
