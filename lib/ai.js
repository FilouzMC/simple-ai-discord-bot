...
export async function generateAnswer({ userQuestion, channelContext, debug, modelOverride = null }) {
  ...
  if (debug) {
  try { console.log(`[#debug][prompt] model=${modelOverride||CURRENT_MODEL} length=${prompt.length}\n----- PROMPT START -----\n${prompt}\n----- PROMPT END -----`); } catch {}
  }
  try {
  const model = genAI.getGenerativeModel({ model: modelOverride || CURRENT_MODEL });
